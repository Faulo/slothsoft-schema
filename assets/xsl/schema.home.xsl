<?xml version="1.0" encoding="UTF-8"?>
<xsl:stylesheet version="1.0" xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ssv="http://schema.slothsoft.net/schema/versioning"
    xmlns:sfd="http://schema.slothsoft.net/farah/dictionary">

    <xsl:include href="farah://slothsoft@schema/xsl/schema.global" />

    <xsl:template match="ssv:info" mode="body">
        <p>
            <xsl:value-of select="ssv:description" />
        </p>
        <p sfd:dict="">home.table.caption</p>
        <table>
            <thead>
                <tr>
                    <th sfd:dict="">home.table.version</th>
                    <th sfd:dict="">home.table.revision</th>
                    <th sfd:dict="">home.table.documentation</th>
                    <th sfd:dict="">home.table.download</th>
                </tr>
            </thead>
            <tbody>
                <xsl:for-each select="$infoList">
                    <xsl:sort select="ssv:version" order="descending" data-type="number" />
                    <xsl:variable name="file" select="concat(ssv:prefix, '-', ssv:revision, '.xsd')" />
                    <tr>
                        <td class="version">
                            <xsl:value-of select="ssv:version" />
                        </td>
                        <td class="version">
                            <xsl:value-of select="ssv:revision" />
                        </td>
                        <td>
                            <a href="{ssv:version}/">
                                <xsl:apply-templates select="." mode="name" />
                            </a>
                        </td>
                        <td>
                            <a href="{ssv:version}.xsd" download="{$file}">
                                <xsl:value-of select="$file" />
                            </a>
                        </td>
                    </tr>
                </xsl:for-each>
            </tbody>
        </table>
        <xsl:if test="ssv:changelog">
            <p sfd:dict="">home.changelog</p>
            <xsl:apply-templates select="." mode="changelog" />
        </xsl:if>
    </xsl:template>
</xsl:stylesheet>